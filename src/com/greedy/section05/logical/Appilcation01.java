...
public class Appilcation01 {
	public static void main(String[] args) {
		/*논리 연산자
		 * 1. 논리 연결 연산자 : 두개의 피연산자를 가지는 이항 연산자이며, 연산자의 결합 방향은 왼쪽에서 오른쪽이다.
		 * 						두 개의 논리식을 판단하여 참과 거짓을 판단한다.
		 * 
		 *  1-1. &&(논리 AND) : 두 개의 논리식 모두 참 일 경우 참을 반환, 둘 중 한 개라도 거짓인 경우 거짓을 반환하는 연산자이다.
		 *  1-2. ||(논리 OR) :  두 개의 논리식 중 둘중 하나라도 참일 경우 참을 반환, 둘다 모두 거짓일 경우 거짓을 반환한다.
		 *  
		 *  2. 논리 부정 연산자 : 피연산자가 하나만 단향연산자로, 피연산자의 결합 방법은 왼쪽에서 오른쪽이다.
